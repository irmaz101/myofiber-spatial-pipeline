...
def main():
    ...
    keep = (expression.sum(axis=0) > 0) & (expression.var(axis=0) > 0)
    ...
    dataset.acquire_weight(k=min(args.neighbors, adata.n_obs - 1))
    ...
    results = pd.DataFrame({
        ...
        "Di_mean": np.asarray(dataset.Di.mean(axis=0)).ravel(),
    })
    results = results.sort_values("AI", ascending=False).reset_index(drop=True)
    results["rank_ai"] = np.arange(1, len(results) + 1)
    results["is_svg"] = results["rank_ai"] <= args.top
