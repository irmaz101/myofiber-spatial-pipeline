...
def generate_random_patch(image, patch_size):
    ...
    x = randint(0, width - patch_size - 1)
    y = randint(0, height - patch_size - 1)
...
def save_patch(patch, sample_name, output_dir, idx_string):
    """Save a patch if it does not already exist."""

    patch_path = output_dir / f"{sample_name}{idx_string}.tif"
