...
The study used 12 randomly selected `1800 × 1800` pixel patches for each
segmentation task. In the supplied script, `patches_per_image = 12`; when more
than one TIFF is present, this means 12 patches are generated **per input
image**. Run the script separately, or adjust this setting, to reproduce a
fixed total of 12 patches for one fibre task and one nuclear task. Cellpose may
internally sample smaller tiles from the annotated regions during training.
...
Before running, edit `mode`, `sample`, `patch_dir` and `label_dir` in the script.
The defaults are examples and do not directly consume `training_patches/`.
The selected patches are opened in Napari for manual annotation.
...
To use a custom trained model, provide the path to the model weights. Also set
`input_dir` and `output_dir` in the script; their defaults are examples:
...
`Pipeline/1_preprocessing/preprocessing.py` matches transcript and mask files
by an identical basename. Because this segmentation script appends
`_mask_original_size`, rename or copy the selected masks before preprocessing,
for example:

```text
cellpose_output/sample_01_mask_original_size.tif
