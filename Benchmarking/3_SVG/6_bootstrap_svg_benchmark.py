...
def bootstrap_metric(
    ...
    complete_mask = scores.notna().all(axis=1)
    excluded_datasets = scores.index[~complete_mask].astype(str)
    scores = scores.loc[complete_mask].copy()

    pd.DataFrame(
        {"dataset": excluded_datasets}
    ).to_csv(
        output_dir / f"{metric_name}_excluded_datasets.csv",
        index=False,
    )

    if scores.empty:
        raise ValueError(
            f"No complete datasets remain for {metric_name}."
        )
