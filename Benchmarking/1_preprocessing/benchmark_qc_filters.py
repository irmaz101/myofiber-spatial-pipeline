"""
Benchmark total-count and transcript-density QC filters.

The script summarizes correlations, overlap between removed objects,
and creates diagnostic plots for each sample.
"""
...
def analyze_sample(csv_path):
    ...
    sample = csv_path.stem.replace(
        "_filtering_comparison",
        "",
    )
    ...
    summary = {
        ...
        "removed_overlap_vs_count": (
            n_removed_both / n_removed_count
            if n_removed_count
            else 0
        ),
        "removed_overlap_vs_density": (
            n_removed_both / n_removed_density
            if n_removed_density
            else 0
        ),
        "correlation_counts_area": data[
            "total_counts"
        ].corr(data["cell_area"]),
        "correlation_density_area": data[
            "transcript_density"
        ].corr(data["cell_area"]),
        "correlation_counts_density": data[
            "total_counts"
        ].corr(data["transcript_density"]),
    }

    data["filter_category"] = "kept_by_both"
    data.loc[
        removed_count & removed_density,
        "filter_category",
    ] = "removed_by_both"
    data.loc[
        removed_count & keep_density,
        "filter_category",
    ] = "removed_only_by_count"
    data.loc[
        keep_count & removed_density,
        "filter_category",
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / f"{sample}_counts_vs_area.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / f"{sample}_density_vs_area.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / f"{sample}_filter_categories.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def main():
    comparison_files = sorted(
        input_dir.glob("*_filtering_comparison.csv")
    )

    if not comparison_files:
        raise FileNotFoundError(
            f"No filtering-comparison files found in "
            f"{input_dir.resolve()}."
        )
    ...
    if results.empty:
        raise ValueError(
            "No valid filtering-comparison files were processed."
        )

    results.to_csv(
        output_dir / "qc_filter_metrics_per_sample.csv",
        index=False,
    )
    ...
    overall.to_csv(
        output_dir / "qc_filter_metrics_summary.csv",
        index=False,
    )
